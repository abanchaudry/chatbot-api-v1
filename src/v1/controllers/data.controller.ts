...
async function embedChunksInBatches(
  ...
  for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
    const batchTexts = chunks.slice(i, i + BATCH_SIZE).map((x) => x.content);
...
async function buildVectorizeBatch(args: {
  ...
  embeddingModel: string;
  chunkMethod: string;
  chunks: Array<{ index: number; content: string; section: string; tags: string[]; topic: string }>;
  vectors: number[][];
...
export const DataController = {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (strategy === "general") {
          ...
          const result = await new ChunkingServiceV2(key).process(rawText, fileName, version, true, uploadId);
        ...
        if (uploadId) await pt.update(uploadId);
      }
    ...
    try {
      for (const f of files) {
        ...
        if (chunkMethod === "general") {
          ...
          const out = await new ChunkingServiceV2(key).process(rawText, fileName, version, true, uploadId || undefined);
  ...
  finalizeChunks: async (c: Context) => {
    ...
    try {
      ...
      // VALIDATE chunks for quality (no data loss, sentence-safe, no fragments)
      const validation = ChunkValidator.validate(normalized, rawText || "");
      if (!validation.valid) {
        const errorCount = validation.issues.filter((i) => i.severity === "error").length;
        const warnMsg = `Chunk quality validation found ${errorCount} critical issues (coverage: ${(validation.coverage.percentageOfSource * 100).toFixed(1)}%). Please review chunk boundaries.`;
        return c.json({ ok: false, message: warnMsg, validation }, 400);
      }
...
getAllChunks: async (c: Context) => {
  ...
  deleteFile: async (c: Context) => {
    ...
    try {
      const file = await fileDb.getFileById(c.env.DB, fileId);
      if (!file) {
      ...
      const fileName = (file as any).file_name || "unknown";

      await fileDb.updateFile(c.env.DB, fileId, { file_status: "deleting", error_message: null });
      ...
      await ingestDb.log(
        ...
        fileId,
        "INFO",
        `Vectorize delete ok: deleted=${vectorsDeleted}, expected=${totalChunkIds}`
      );

      // Safety check: do not delete SQL if vector delete mismatch
      if (vectorsDeleted !== totalChunkIds) {
        const msg = `Vector delete mismatch (deleted=${vectorsDeleted}, expected=${totalChunkIds})`;
        await fileDb.updateFile(c.env.DB, fileId, { file_status: "delete_failed", error_message: msg });
        await ingestDb.log(c.env.DB, jobId, fileId, "ERROR", msg);
        await ingestDb.finishJob(c.env.DB, jobId, "failed", msg);
        return c.json({ ok: false, message: msg, fileId, vectorsDeleted, totalChunkIds }, 500);
      }

      await fileDb.deleteChunksByFileId(c.env.DB, fileId);
      await c.env.DB.prepare(`DELETE FROM files WHERE file_id = ?`).bind(fileId).run();

      await ingestDb.log(c.env.DB, jobId, fileId, "INFO", "D1 delete ok: file + chunks removed");
      await ingestDb.finishJob(c.env.DB, jobId, "completed");
      ...
      try {
        await fileDb.updateFile(c.env.DB, fileId, {
          ...
          error_message: `Delete failed: ${err.message}`,
        });
      } catch {}

      try {
        await ingestDb.log(c.env.DB, jobId, fileId, "ERROR", `Delete failed: ${err.message}`);
        await ingestDb.finishJob(c.env.DB, jobId, "failed", err.message);
      } catch {}
