...
function buildHistoryPreview(rows: Array<{ question?: string; answer?: string }>) {
  return rows
    ...
    .join("\n\n")
    .slice(0, 6500); // Ask-helper.LIMITS.historyPreviewChars
}
...
/**
 * Stage 1: Validate input payload and setup user/thread
 */
...
/**
 * Stage 2: Load conversation history
 */
async function stageLoadHistory(
  ...
  traceStepEnd(trace, "history_preview", 0, { previewChars: historyPreview.length });
