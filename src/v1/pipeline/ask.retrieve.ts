...
import { retrieveLocalCandidates, expandParentChunks } from "../utils/local-retriever";
...
async function executeSingleRetrievalPass(args: {
  ...
  const rawDeduped = dedupePiecesByKey(fusion.candidates);

  // Phase 4: Parent-Child Hierarchical Context Expansion (Leaf -> Parent Context)
  const dedupedPieces = await expandParentChunks(db, rawDeduped);
