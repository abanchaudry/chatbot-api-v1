...
export const authdb = {
  ...
  async saveUser(db: D1Database, username: string, password: string): Promise<void> {
    await db
      .prepare(
        `INSERT INTO auth (username, password, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)`
      )
  ...
  async getUserById(db: D1Database, userId: string): Promise<SafeAuthUser | null> {
    const result = await db
      .prepare(`SELECT id, username, created_at, last_login FROM auth WHERE id = ?`)
      .bind(userId)
      .first();
  ...
  async getAllAuthUsers(db: D1Database): Promise<SafeAuthUser[]> {
    const result = await db
      .prepare(`SELECT id, username, created_at, last_login FROM auth ORDER BY created_at DESC`)
      .all();
