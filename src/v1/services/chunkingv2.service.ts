...
const DEFAULTS: Required<Omit<ChunkerOptions, "minCoverageRatioWarn">> & { minCoverageRatioWarn: number } = {
  chunkModel: "gpt-4o",
...
export class ChunkingServiceV2 {
  ...
  async process(
    ...
    if (uploadId) progressTracker.step(uploadId, `Starting chunking for ${fileName}`);
    ...
    if (uploadId) progressTracker.step(uploadId, `Content cleaned: removed ${quality.artifactsRemoved} artifacts`);
    ...
    if (uploadId) progressTracker.step(uploadId, `Chunks generated: ${chunks.length}`);
    ...
    if (uploadId) progressTracker.step(uploadId, `Chunk quality improved: boundaries fixed, fragments merged`);
    ...
    if (uploadId && !preview) progressTracker.step(uploadId, `Embeddings generated`);
...
Document:\n${batches[i]}`;
      ...
      if (uploadId) progressTracker.step(uploadId, `${batchLabel} started`);
      ...
      while (true) {
        try {
          ...
          totalChunks.push(...arr);

          if (uploadId) progressTracker.step(uploadId, `${batchLabel} done (${arr.length} chunks)`);
          if (uploadId) progressTracker.update(uploadId);
          ...
          if (attempt >= this.opts.maxRetries) {
            ...
            if (uploadId) progressTracker.fail(uploadId, errorMsg);
            throw new Error("chunking failed");
          }
          const wait = jitter(500, attempt);
