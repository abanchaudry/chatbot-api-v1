...
### 📚 2. Admin-Only Cited Knowledge Documents & Passage Snippets
- **Real-Time Citation Pipeline**: Every response generated by `/ask` or `/ask/stream` extracts and formats RAG retrieval sources (`section`, `fileName`, `score`, `snippet`).
- **Admin Chatbot Widget Integration**: Under every bot query response in the Admin Chatbot portal, a sleek **📚 Cited Knowledge Documents** section displays clickable badges with relevance match percentages (e.g. `87% match`).
- **Interactive Passage Snippet Modal**: Clicking any cited document pill opens a popup preview modal displaying the exact document name, topic, match score, and text passage used by the RAG engine.
...
│       └── shared/                       # Admin Chatbot with Cited Docs & Passage Snippet Modal
│
